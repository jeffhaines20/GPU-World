...
- **`HANDOFF_PROMPT.md`** — the message to paste to the agent that will run the loop, with the numbers
  filled in.
- **`kit/`** — everything the agents run from: the orchestrator's rules, twelve agent briefs, seven
  skills, ten checks with a suite of sixty-eight planted faults and fifteen positive controls, and the templates the loop fills in.
