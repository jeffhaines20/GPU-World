...
cards = {m.group(1): m.group(0) for m in re.finditer(r"^### ([PQ]-\d\d) — cell [A-Za-z0-9]+ — .*?(?=^### [PQ]-\d\d — |\Z)", text, re.M | re.S)}
parts = []
for i in ids:
    if i not in cards:
        sys.exit(f"no card {i}")
    card = re.sub(r"^(### [PQ]-\d\d) — cell [A-Za-z0-9]+ — ", r"\1 — ", cards[i].strip())   # judges do not receive the writers' cells
    if probes != "-":
