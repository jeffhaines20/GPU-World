...
|---|---|---|---|
| Opening | `opening` | Which story earns the next page faster, and how? | |
| Character | `character` | In which story does a person want something, and pay for it? | |
| The idea | `idea` | Which story's science-fictional idea is stranger and better used — not which idea is cleverer, but which is more *used*? | |
| Scene | `scene` | Which story puts you in a place with people doing things, rather than telling you about it? | |
| Sentences | `sentences` | Which story's prose would you rather read for an hour? | |
| The turn | `turn` | In which story does something change that cannot change back? | |
| The ending | `ending` | Which ending is both surprising and inevitable? Which one earns its last image? | |
| Aftertaste | `aftertaste` | A day later, which one would you still be thinking about, and about what? | |
