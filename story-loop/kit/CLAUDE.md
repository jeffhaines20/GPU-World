...
**Why `claude/` has no leading dot.** The agent briefs and skills live in `claude/agents/` and
`claude/skills/`, not `.claude/`. A dot-prefixed folder is hidden by Finder, Explorer and most file
managers, and some upload and drag-and-drop flows drop it silently — which would take the twelve briefs
and seven skills with it and leave a kit that looks complete and has no behaviour. The loop never relied
on a harness auto-discovering that path: rule 1 says an agent is spawned by handing its file to the
sub-agent facility as the brief, and a skill is a file the orchestrator reads. Do not rename it back.
